...
#####  1.1.1 极限 $\displaystyle \lim_{x\rightarrow x_{0}} f(x) = A$ <font color=red>直观的定义</font>
...
##### 1.1.2 <font color=red>定义1</font> .极限的 $\varepsilon -\delta$ 定义（极限 $\displaystyle \lim_{x \rightarrow x_{0}}f(x) = A$ 的<font color=red>严格定义</font>）
...
##### 1.1.5 用定义证明极限 $\displaystyle \lim_{x \rightarrow x_{0}} f(x) = A$ 的方法

对于任意给定的 $\varepsilon > 0 $，

从 $|f(x) - A| < \varepsilon $ 分析出 $|x - x_{0}| < \varphi(\varepsilon)$

令 $\delta = \varphi(\varepsilon)$ ，则当 $0 < |x - x_{0}| < \delta$ 时

就有 $|f(x) - A| < \varepsilon$



(1) 适当放大法，在比较过程中，不好消除元素时，可分析 x 趋于的数，对不好消除的式子进行适当放大，来得出结果。注意在判断 $\delta$ 时，需要把适当放大时候的不等式也加入一起判断，取较小者为 $\delta$ 。
